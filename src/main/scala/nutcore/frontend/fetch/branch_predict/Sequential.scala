...
class Sequential extends NutCoreModule {
  ...
  // BTB
  val NRbtb = 512
  val btbAddr = new EntryAddr(log2Up(NRbtb))
  ...
  // Should be replaced with signal input
  ...
  Debug(req.valid, "[BTBUP] pc=%x tag=%x index=%x bridx=%x tgt=%x type=%x\n", req.pc, btbAddr.getTag(req.pc), btbAddr.getIdx(req.pc), Cat(req.pc(1), ~req.pc(1)), req.actualTarget, req.btbType)
    ...
    //  Debug("%d\n", GTimer())
    //  Debug("[BTBHT6] btbWrite.type is BranchType.R/RET!!! Inpc:%x btbWrite.brIdx:%x setIdx:%x\n", io.in.pc.bits, btbWrite.brIdx, btb.io.w.req.bits.setIdx)
